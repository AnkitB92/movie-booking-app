...
const SearchTab = ({ onFilterChange }) => {
    ...
    const [fullCinemaData, setFullCinemaData] = useState([]);
    ...
    const fetchCinemaOptions = async () => {
        try {
            ...
            if (response.status === 200) {
                ...
                setFullCinemaData(cinemaData); // Store full cinema data
    ...
    const handleOptionChange = (e, setterFunction, option) => {
        const selectedValue = e.target.value;
        setterFunction(selectedValue);

        if (option === 'city') {
            if (selectedValue) {
                const filteredCinemas = fullCinemaData.filter(cinema => cinema.city === selectedValue);
                const uniqueCinemaNames = [...new Set(filteredCinemas.map((cinema) => cinema.name))];
                setCinemaOptions(uniqueCinemaNames);
            } else {
                const uniqueCinemaNames = [...new Set(fullCinemaData.map((cinema) => cinema.name))];
                setCinemaOptions(uniqueCinemaNames);
            }
        }
        ...
        onFilterChange({
            keyword,
            city: option === 'city' ? selectedValue : city,
            cinema: option === 'cinema' ? selectedValue : cinema,
            genre: option === 'genre' ? selectedValue : genre
        });
    };
